...
bool GenericVerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, unsigned int flags, const T& data)
{
    ...
    return VerifyScript(scriptSig, scriptPubKey, flags, SimpleSignatureChecker(SerializeHash(data)), &serror);
}
...
template<typename T>
CScript GenericCombineSignatures(const CScript& scriptPubKey, const T& data, const CScript& scriptSig1, const CScript& scriptSig2)
{
