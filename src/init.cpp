    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Check for changed -opreturnindex state
                if (fOpReturnIndex != GetBoolArg("-opreturnindex", DEFAULT_OPRETURNINDEX)) {
                    strLoadError = _("You need to rebuild the database using -reindex to change -opreturnindex");
                    break;
                }
